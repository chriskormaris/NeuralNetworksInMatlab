function [W1, W2, cost_vector] = ml_softmaxTrain(T, X, lambda, W1init, W2init, options, activation_function)
%
% What it does: It trains using gradient descent a neural network
...
% batch or minibatch gradient descent
...
if batch_or_minibatch == 1
    ...
    disp('Running batch gradient descent...')
    
    maxiter = options(1);
    Ewold = -Inf;
    cost_vector = zeros(maxiter, 1);
    for it=1:maxiter
...
elseif batch_or_minibatch == 0
    ...
    disp('Running mini-batch gradient descent...')

    maxepochs = options(1);
    batch_size = options(5);
    s_old = -Inf;
    cost_vector = zeros(maxepochs, 1);
    for ep=1:maxepochs
        ...
        if nargin >= 6
            ...
            for i=1:iterations
                ...
                % Update parameters based on minibatch gradient descent
                W1 = W1 - eta * gradEw1i;
                W2 = W2 - eta * gradEw2i;
            end
        end
