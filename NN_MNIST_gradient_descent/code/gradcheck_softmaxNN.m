...
% Compute the analytic gradients and store them in gradEw1 and gradEw2
%
if nargin >= 5
    if nargin == 5
...
if nargin >= 5
    ...
    for i=1:M
        for j=1:D+1
            ...
            %fprintf('Gradcheck for parameter W1, iteration: %g\n', ((i-1)*M + j));
			%if isOctave,
			%	fflush(stdout);
			%end
        end
...
if nargin >= 5
    ...
    for i=1:K
        for j=1:M+1
            ...
            %fprintf('Gradcheck for parameter W2, iteration: %g\n', ((i-1)*M + j));
			%if isOctave,
			%	fflush(stdout);
			%end
        end
